...
class TestGateUserAppAssocationAPI(ApiTestCase):
    ...
    def test_user_grp_assoc_api_create(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_001 test_user_grp_assoc_api_create
        ...
        #verify API
        self.assertUserGrpData(create_response.json(), user_grp_data)
        # clean up
        del_response = self.gk_service.gk_crud(
            session,
            method='DELETE',
            resource="user_grp",
            id=user_grp_data['user_id'],
            id2=user_grp_data['group_id']
        )
        ...
        del_response = self.gk_service.gk_crud(
            ...
            method='DELETE',
            resource="group",
            id=user_grp_data['group_id']
        )
        # ensure a 204 is returned
        self.assertEquals(del_response.status_code, requests.codes.no_content)

    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_miss_params(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_002 test_user_grp_assoc_api_miss_params
        ...
        for n_dict in no_data:
            data = self.gk_service.create_user_grp_data(session, n_dict)
            if data['group_id']:
                group_id = data['group_id']
            if data['user_id']:
                user_id = data['user_id']
            create_response = self.gk_service.gk_crud(
                session, method='POST', resource="user_grp", data=data
            )
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_no_data(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_003 test_user_grp_assoc_api_no_data
        ...
        no_data = {'group_id': None, 'user_id': None}

        create_response = self.gk_service.gk_crud(
            session, method='POST', resource="user_grp", data=no_data
        )
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_create_dup(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_004 test_user_grp_assoc_api_create_dup
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_no_update(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_005 test_user_grp_assoc_api_no_update
        update not allowed,
        clean up the data (implictly tests DELETE and GET)
        """
        # login and create session
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_read(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_006 test_user_grp_assoc_api_read
        read the data
        ...
        self.assertUserGrpData(read_response.json(), user_grp_data)
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_read_not_exis(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_007 test_user_grp_assoc_api_read_not_exis
        ...
        non_existant_data = [
            ...
            {'group_id': user_grp_data['group_id'],
             'user_id': self.util.random_int()},
            {'group_id': self.util.random_int(),
             'user_id': user_grp_data['user_id']},
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_read_no_data(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_008 test_user_grp_assoc_api_read_no_data
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_delete(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_009 test_user_grp_assoc_api_delete
        delete assocation
        """
        # login and create session
        session, cookie_id, response = self.gk_service.login_create_session(
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_del_no_data(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_010 test_user_grp_assoc_api_del_no_data
    ...
    @attr(env=['test'], priority=1)
    def test_user_grp_assoc_api_del_not_exist(self):
        """
        GATEKEEPER_USER_GRP_ASSOC_API_011 test_user_grp_assoc_api_del_not_exist
