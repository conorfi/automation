...
class TestGateKeeperUsersAppsListingAPI(unittest.TestCase):
    ...
    def test_user_apps_api(self):
        """
        GATEKEEPER_USER_APPS_API_001 test_user_apps_api
        Ensure all the user_app information stored is returned
        """
        ...
        db_count = self.gk_dao.get_user_app_count(self.db)['count']
        self.assertEquals(api_count, db_count, "count mismatch")

    @attr(env=['test'], priority=1)
    def test_user_apps_filter(self):
        """
        ATEKEEPER_USER_APPS_API_002 test_user_apps_filter
    ...
    def test_users_apps_api_filter_no_data(self):
        """
        GATEKEEPER_USER_APPS_API_003 test_users_apps_api_filter_no_data
    ...
    def test_users_apps_api_filter_invalid_data(self):
        """
        GATEKEEPER_USER_APPS_API_004 test_users_apps_api_filter_invalid_data
        Ensure the name filer works correctly when no
        or invalid data is entered
        """
        ...
        dict_matrix = [
            ...
            {'user_id': rand_int, 'application_id': rand_int}
        ]
