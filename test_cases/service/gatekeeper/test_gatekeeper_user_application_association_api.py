"""
@summary: Contains a set of test cases for the user application API
of the gatekeeper(single sign on) project
These test cases realted to the association between user_id and applciation_id
...
class TestGateUserAppAssocationAPI(ApiTestCase):
    ...
    def test_user_app_assoc_api_create(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_001 test_user_app_assoc_api_create
        ...
        user_app_data = self.gk_service.create_user_app_data(session)

        # create an association
        create_response = self.gk_service.gk_crud(
            session, method='POST', resource="user_app", data=user_app_data
        )
        # ensure a 201 is returned
        self.assertEquals(create_response.status_code, requests.codes.created)
        #verify api
        self.assertUserAppData(create_response.json(), user_app_data)
        ...
        del_response = self.gk_service.gk_crud(
            ...
            method='DELETE',
            resource="user_app",
            id=user_app_data['user_id'],
            id2=user_app_data['application_id']
        )
        ...
        del_response = self.gk_service.gk_crud(
            ...
            id=user_app_data['application_id']
        )
        # ensure a 204 is returned
        self.assertEquals(del_response.status_code, requests.codes.no_content)

    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_miss_params(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_002 test_user_app_assoc_api_miss_params
        ...
        for n_dict in no_data:
            data = self.gk_service.create_user_app_data(session, n_dict)
            if data['application_id']:
                application_id = data['application_id']
            if data['user_id']:
                user_id = data['user_id']
            create_response = self.gk_service.gk_crud(
                session, method='POST', resource="user_app", data=data
            )
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_no_data(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_003 test_user_app_assoc_api_no_data
        ...
        no_data = {'application_id': None, 'user_id': None}

        create_response = self.gk_service.gk_crud(
            session, method='POST', resource="user_app", data=no_data
        )
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_create_dup(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_004 test_user_app_assoc_api_create_dup
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_no_update(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_005 test_user_app_assoc_api_no_update
        update not allowed,
        clean up the data (implictly tests DELETE and GET)
        """
        # login and create session
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_read(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_006 test_user_app_assoc_api_read
        read the data
        ...
        self.assertUserAppData(read_response.json(), user_app_data)
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_read_not_exis(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_007 test_user_app_assoc_api_read_not_exis
        ...
        non_existant_data = [
            ...
            {'application_id': user_app_data['application_id'],
             'user_id': self.util.random_int()},
            {'application_id': self.util.random_int(),
             'user_id': user_app_data['user_id']}
        ]
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_read_no_data(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_008 test_user_app_assoc_api_read_no_data
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_delete(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_009 test_user_app_assoc_api_delete
        delete assocation
        """
        # login and create session
        session, cookie_id, response = self.gk_service.login_create_session(
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_del_no_data(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_010 test_user_app_assoc_api_del_no_data
        ...
        # delete with 2 non existant ids
        read_response = self.gk_service.gk_crud(
            session,
            method='DELETE',
            resource="user_app",
    ...
    @attr(env=['test'], priority=1)
    def test_user_app_assoc_api_del_not_exist(self):
        """
        GATEKEEPER_USER_APP_ASSOC_API_011 test_user_app_assoc_api_del_not_exist
